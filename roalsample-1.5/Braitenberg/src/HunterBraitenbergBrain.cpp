...
#include "HunterBraitenbergBrain.h"
...
//------------------------------------------------------------------------------
HunterBraitenbergBrain::HunterBraitenbergBrain(const std::string name)
:Brain(name)
{
    age = 0;
    this->energy = 10000;
...
//------------------------------------------------------------------------------
HunterBraitenbergBrain::~HunterBraitenbergBrain()
{
...
//------------------------------------------------------------------------------
void HunterBraitenbergBrain::iterate(float elapsedTime)
{
    ...
    // check if the vehicle reaches a victim
    if ((lf > 0.85|| rf > 0.85 || lb > 0.85 || rb > 0.85) && (c1+c2+c3+c4 > 0.0))
    {
        addEnergy(c1+c2+c3+c4);
...
//------------------------------------------------------------------------------
void HunterBraitenbergBrain::registerBody(Body* body)
{
    this->body = body;    
}

//------------------------------------------------------------------------------
float HunterBraitenbergBrain::getEnergy()
{
    return this->energy;
}
...
//------------------------------------------------------------------------------
void HunterBraitenbergBrain::addEnergy(float energy)
{
